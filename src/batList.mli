...
##V>=4.12##    the two input lists have the same length, and for each
##V>=4.12##    pair of elements [ai], [bi] at the same position we have
##V>=4.12##    [eq ai bi].
##V>=4.12##
##V>=4.12##    Note: the [eq] function may be called even if the
##V>=4.12##    lists have different length. If you know your equality
##V>=4.12##    function is costly, you may want to check {!compare_lengths}
##V>=4.12##    first.
##V>=4.12##
...
##V>=4.07##val to_seq : 'a list -> 'a Seq.t
##V>=4.07##(** Iterate on the list
##V>=4.07##    @since 2.10.0 and OCaml 4.07 *)

##V>=4.07##val of_seq : 'a Seq.t -> 'a list
...
(** [List.iter2 f [a0; a1; ...; an] [b0; b1; ...; bn]] calls in turn
    ...
    @raise Invalid_argument if two lists have different lengths. *)
...
##V>=4.10##(** [find_map_opt f l] applies [f] to the elements of [l] in order,
...
(** [count_matching p l] returns the number of elements in [l]
...
val partition_map : ('a -> ('b, 'c) BatEither.t) -> 'a list ->
  'b list * 'c list
...
(** [modify a f l] returns the same list as [l] but with value [b] associated
    to key [a] replaced with [f b].
...
(** [split_at n l] returns two lists [l1] and [l2], [l1] containing the
    ...
    @raise Invalid_argument if [n] is outside of [l] size bounds. *)
...
(** [fold_while p f init l], accumulates elements [x] of list [l] using
...
##V>=4.10##val concat_map : f:('a -> 'b list) -> 'a list -> 'b list
  val find_all : f:('a -> bool) -> 'a list -> 'a list
  val partition : f:('a -> bool) -> 'a list -> 'a list * 'a list
  val partition_map : f:('a -> ('b, 'c) BatEither.t) -> 'a list ->
    'b list * 'c list
