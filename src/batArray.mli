...
val fold_while : ('acc -> 'a -> bool) -> ('acc -> 'a -> 'acc) -> 'acc -> 'a array -> ('acc * int)
(** [fold_while p f init a], accumulates elements [x] of array [a] using
    function [f], as long as the predicate [p acc x] holds.
    At the end, the accumulated value along with the first index i
    where [p acc a.(i)] does not hold is returned.
    If the returned index is equal to [length a], the whole array
    was folded. *)
...
(** [pivot_split cmp arr x] assumes that [arr] is {b sorted} w.r.t
    ...
    @raise Invalid_argument if the array is found not to be sorted
*)
...
(** [find_opt p a] returns the first element of the array [a] that satisfies
    the predicate [p], or [None] if there is no such element.
    @since 3.4.0 *)
...
(** [count_matching p a] returns the number of elements of [a]
 ...
 *)
  ...
  module Labels : sig
    ...
    val fold      : f:('a -> 'b -> 'a) -> init:'a ->  ('b, [> `Read]) t -> 'a
    ...
    val find_opt:   f:('a -> bool) -> ('a, [> `Read]) t -> 'a option
    val find_map:   f:('a -> 'b option) -> ('a, [> `Read]) t -> 'b option
    val map:        f:('a -> 'b) -> ('a, [>`Read]) t -> ('b, _) t
    val mapi:       f:(int -> 'a -> 'b) -> ('a, [>`Read]) t -> ('b, _) t
...
##V>=4.6##module Floatarray : sig
##V>=4.6##  external create : int -> floatarray = "caml_floatarray_create"
##V>=4.6##  external length : floatarray -> int = "%floatarray_length"
##V>=4.6##  external get : floatarray -> int -> float = "%floatarray_safe_get"
##V>=4.6##  external set : floatarray -> int -> float -> unit = "%floatarray_safe_set"
##V>=4.6##  external unsafe_get : floatarray -> int -> float = "%floatarray_unsafe_get"
